...
def tables(request):
  ...
  pannes = Anomalie.objects.all()
  if request.method == "POST":
    request.POST.get('nomequip')
    request.POST.get('codeequip')
    request.POST.get('localequip')
    request.POST.get('codetech')
    Equipement.objects.create(nomEquipement=request.POST.get('nomequip'), codeEquipement= request.POST.get('codeequip'), site= request.POST.get('localequip'), matRespoEntretien_id=
    request.POST.get('codetech'))
    return render(request, 'pages/tables.html', { 'segment': 'tables', 'users': users, 'equipments': equipments, 'pannes': pannes})
  else:
    return render(request, 'pages/tables.html',
                {'segment': 'tables', 'users': users, 'equipments': equipments, 'pannes': pannes})
